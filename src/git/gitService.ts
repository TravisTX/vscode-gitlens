...
export class GitService implements Disposable {
	...
	async getMergeStatus(repoPath: string): Promise<MergeStatus | undefined>;
	async getMergeStatus(status: GitStatus): Promise<MergeStatus | undefined>;
	@log({ args: false })
	async getMergeStatus(repoPathOrStatus: string | GitStatus): Promise<MergeStatus | undefined> {
		let status;
